...
function ready() {
    ...
    for (var k = 0; k < addToCartButtons.length; k++) {
        var button1 = addToCartButtons[k]
...
function updateCartTax() {
    ...
    for (var i = 0; i < cartRows.length; i++) {
        ...
        tax = tax + (price * quantity)*9.25/100
    }
...
function updateCartTotal() {
	...
	if(cartRows.length == 0){
		...
		document.getElementsByClassName('cart-total-sum')[0].innerText = '$' + sum
	}
	else{
        for (var i = 0; i < cartRows.length; i++) {
			var cartRow = cartRows[i]
            ...
            var price = parseFloat(priceElement.innerText.replace('$', ''))
            var quantity = quantityElement.value
            total = total + (price * quantity)*1.09
			sum = sum + (price * quantity)
		}
		sum = Math.round(sum * 100) / 100
		total = Math.round(total * 100) / 100 + 15
		document.getElementsByClassName('cart-total-price')[0].innerText = '$' + total
		document.getElementsByClassName('cart-total-sum')[0].innerText = '$' + sum
	}
}